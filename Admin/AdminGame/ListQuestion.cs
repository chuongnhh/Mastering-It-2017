...
namespace Admin.AdminGame
{
    public partial class ListQuestion : Form
    {
        ...
        private Guid gameId;
        public ListQuestion(Guid gameId)
        {
            this.gameId = gameId;
            InitializeComponent();
        }
        ...
        {
            (new InsertQuestion(gameId)).ShowDialog();
            bs.DataSource = (new BLQuestion()).GetAll();
        }

        private void ListQuestion_Load(object sender, EventArgs e)
        {
            bs.DataSource = (new BLQuestion()).GetByGame(gameId);
